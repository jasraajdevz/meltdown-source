...
class Plant {
  ...
  void step(double dt) {
    ...
    var load = 0.0;
    for (final a in kAlarms) {
      if (condition(a.id)) load += a.critical ? 0.16 : 0.06;
    }
    if (damage > 0) load += clamp01(damage / 40) * 0.8;
    if (radiation > 10) load += clamp01(radiation / 100) * 0.6;
    if (hornActive) load += 0.15; // silence it and you think more clearly
    stress = clamp01(load);
    ...
    sanity = clampD(sanity - (0.12 + stress * 1.2) * dt, 0, 100);
...
class Objective {
  const Objective(this.tab, this.text, this.done);
  final int tab;
  final String text;
...
final List<Objective> kObjectives = [
  Objective(1, 'COOLANT panel — start at least 2 reactor coolant pumps',
      (p) => p.pumpCount >= 2),
  Objective(1, 'Let pump heat warm the coolant past 280 °C (watch T-AVG)',
      (p) => p.tAvg > 280),
  Objective(1, 'Set pressurizer heaters HIGH until pressure passes 150 bar',
      (p) => p.pressure > 150),
  Objective(0, 'REACTIVITY panel — press RESET TRIP to release the rods',
      (p) => !p.scramLatched),
  Objective(0, 'Withdraw all four rod banks (hold OUT on each)',
      (p) => p.rodAvg > 90),
  Objective(0, 'Hold DILUTE until the reactor goes critical — keep SUR under 1',
      (p) => p.power > 1e-5),
  Objective(2, 'STEAM panel — open the MSIV and run feedwater above 60%',
      (p) => p.msiv && p.feedPump > 60),
  Objective(2, 'Roll the turbine — open the throttle to about 15%',
      (p) => p.throttle > 12),
  Objective(4, 'ELECTRICAL panel — close the generator breaker while in phase',
      (p) => p.genBreaker),
  Objective(2, 'Raise the throttle and dilute together to reach 50% power',
      (p) => p.power > 0.5),
];
...
const List<ManualSection> kManual = [
  ManualSection(
    ...
    'Fission heats the fuel. The fuel heats pressurised water. That water boils '
        'a separate loop into steam, the steam spins a turbine, and the turbine '
        'sells megawatts to the grid. You are paid for megawatts that actually '
        'reach the grid — heat alone earns nothing.',
    [
      ManualEntry('URANIUM ⬢',
          'Earned every second the generator breaker is closed and steam is flowing.'),
      ManualEntry('RESEARCH ◆',
          'Awarded when a shift ends, based on megawatt-hours generated. Core '
          'damage cuts the award by up to 75%. Research buys permanent upgrades.'),
      ManualEntry('END SHIFT',
          'Bank your work and walk out safely. You never have to melt down.'),
    ],
  ),
  ManualSection(
    'COLD STARTUP — the order that works',
    'A reactor is not a light switch. Heat first, pressure second, fission '
        'third, steam last. The checklist under the mimic walks you through it, '
        'but this is the whole procedure in one place.',
    [
      ManualEntry('1. PUMPS',
          'COOLANT panel: start two reactor coolant pumps. Their mechanical heat '
          'alone warms the loop to about 290 °C over a minute or two — this is '
          'genuinely how a real plant heats up before it ever goes critical.'),
      ManualEntry('2. PRESSURE',
          'Heaters to HIGH until pressure passes 150 bar, then back to LOW to hold it.'),
      ManualEntry('3. RELEASE RODS',
          'Press RESET TRIP. It only works with the rods fully in and power low.'),
      ManualEntry('4. WITHDRAW',
          'Hold OUT on each of the four banks until they read 100%. Nothing '
          'dramatic happens yet — boron is still holding the reactor down.'),
      ManualEntry('5. GO CRITICAL',
          'Hold DILUTE. Watch SUR: about 1 decade per minute is a controlled '
          'startup. Let go and the reactor levels off on its own.'),
      ManualEntry('6. STEAM',
          'Open the MSIV, feed pump to about 70%, then crack the throttle to 15%.'),
      ManualEntry('7. SYNC',
          'Watch the synchroscope crawl, close the breaker near the top, and you '
          'are selling power.'),
      ManualEntry('8. LOAD UP',
          'Raise the throttle and dilute together. More steam pulls more heat out, '
          'which raises reactivity, which raises power. They move as a pair.'),
    ],
  ),
  ManualSection(
    'REACTIVITY — the balance you are really playing',
    'Reactivity is measured in pcm. Positive means power is rising, zero means '
        'steady, negative means falling. Three things move it: control rods '
        '(withdrawing adds reactivity), boron dissolved in the water (adding '
        'absorbs it), and coolant temperature. That last one is your safety net '
        '— hotter coolant automatically reduces reactivity, so the plant settles '
        'itself instead of running away.',
    [
      ManualEntry('BANK A–D',
          'Selects which of the four rod banks the drive controls will move.'),
      ManualEntry('IN / OUT',
          'Hold to drive the selected bank; springs back to HOLD on release. '
          'OUT raises power, IN lowers it.'),
      ManualEntry('BORATE / DILUTE',
          'Borate adds absorber and shuts the reactor down slowly. Dilute removes '
          'it and lets power and temperature climb. Boron for slow broad changes, '
          'rods for fast fine ones.'),
      ManualEntry('SUR',
          'Startup rate in decades per minute — how fast power is moving. Keep it '
          'under about 1.0 during startup or you will overshoot into a trip.'),
      ManualEntry('NEUTRON FLUX',
          'Logarithmic meter spanning SOURCE, INTERMEDIATE and POWER range. The '
          'reactor covers nine decades between shutdown and full power.'),
      ManualEntry('AUTO ROD',
          'With the Reactor Manual Control System installed, rods chase your power setpoint.'),
    ],
  ),
  ManualSection(
    'PRIMARY COOLANT AND PRESSURE',
    'The primary loop carries heat out of the core. Without flow the fuel '
        'overheats within seconds, so pumps come first, always.',
    [
      ManualEntry('RCP 1–4',
          'Reactor coolant pumps. Two is the practical minimum at power; losing '
          'the last one trips the reactor automatically.'),
      ManualEntry('HEATERS',
          'Off / Low / High. Heating the pressurizer raises system pressure. Target 155 bar.'),
      ManualEntry('SPRAY',
          'Sprays cool water into the pressurizer to drop pressure quickly.'),
      ManualEntry('PORV',
          'Power-operated relief valve, guarded. Dumps pressure fast. Leaving it '
          'open bleeds the system dry.'),
      ManualEntry('T-AVG',
          'Average coolant temperature, about 300 °C at power. Past 345 °C trips the plant.'),
    ],
  ),
  ManualSection(
    'STEAM AND FEEDWATER',
    'The secondary loop turns heat into electricity. Feed in as much water as '
        'you boil off, or the steam generator empties and trips you.',
    [
      ManualEntry('FEED PUMP', 'Sets how much water is available to the steam generator.'),
      ManualEntry('FRV',
          'Feedwater regulating valve. In AUTO it matches feed to steam demand and '
          'holds level near 50%. In MANUAL you position it yourself.'),
      ManualEntry('MSIV',
          'Main steam isolation valve. Closed, no steam leaves containment and the '
          'turbine cannot run. Open it before rolling the turbine.'),
      ManualEntry('THROTTLE',
          'Admits steam to the turbine — your real output control. More steam means '
          'more megawatts and more heat pulled out of the core.'),
      ManualEntry('SG LEVEL',
          'Steam generator water level. Hold near 50%; below 12% the reactor trips.'),
    ],
  ),
  ManualSection(
    'ELECTRICAL AND THE GRID',
    'A spinning turbine earns nothing until it is locked in step with the grid.',
    [
      ManualEntry('SYNCHROSCOPE',
          'The needle spins when turbine speed does not match grid frequency. Trim '
          'the throttle until it crawls, then close the breaker as it passes the top.'),
      ManualEntry('GEN BREAKER',
          'Connects the plant to the grid. Closed and steaming means you are being paid.'),
      ManualEntry('EDG', 'Emergency diesel generator — onsite backup power for safety gear.'),
    ],
  ),
  ManualSection(
    'SAFETY SYSTEMS',
    'Everything here exists to stop a bad hour becoming a destroyed core.',
    [
      ManualEntry('SCRAM',
          'Guarded. Drops every rod instantly and trips the turbine. Always the '
          'right answer when you are losing control — it costs production, never the core.'),
      ManualEntry('RESET TRIP',
          'Rods must be fully inserted and power low before the trip will reset.'),
      ManualEntry('SAFETY INJECTION',
          'Floods the core with high-pressure water: raises pressure and level, cools fast.'),
      ManualEntry('CONTAINMENT ISOLATION', 'Seals the building when radiation climbs.'),
      ManualEntry('CONTAINMENT SPRAY',
          'Condenses steam and scrubs airborne activity out of containment.'),
      ManualEntry('AUX FEEDWATER',
          'Backup water to the steam generators when main feed cannot cope.'),
    ],
  ),
  ManualSection(
    'THE FUEL CYCLE — where uranium actually goes',
    'Uranium is not a score. It is fuel, and it is also what the ministry pays '
        'you in. You burn it to make power, the grid pays you an allotment of '
        'it for the megawatt-hours you deliver, and you spend that allotment on '
        'equipment, the canteen, and the next core load. That is the whole loop.',
    [
      ManualEntry('BURNUP',
          'How far through the current fuel load you are. Fission consumes fuel '
          'and builds poisons, so reactivity falls steadily as burnup climbs — '
          'which is exactly why you dilute boron a little further every shift.'),
      ManualEntry('END OF LIFE',
          'Past about 85% the core cannot hold full power no matter how far you '
          'dilute. That is the plant telling you to refuel, not a bug.'),
      ManualEntry('REFUELLING OUTAGE',
          'Taken from the control building between shifts. Costs uranium in '
          'proportion to how much of the core you are replacing, and hands you '
          'a fresh one. A meltdown refuels you the hard way.'),
      ManualEntry('DISPATCH TARGET',
          'The grid dispatcher asks for a load in MWe and changes it through '
          'the watch. Sitting on it pays up to 1.45x; ignoring it pays 0.6x. '
          'The top strip shows what you are sending against what was asked.'),
    ],
  ),
  ManualSection(
    'THE SHIFT LOG',
    'The control room keeps a narrative log, and so does this one. Trips, '
        'resets, dispatch calls and everything the building does at night are '
        'written down with a wall-clock time. The newest entry takes over the '
        'status strip for a few seconds; the full log lives on the CREW panel.',
    [
      ManualEntry('WALL CLOCK',
          'You are on nights. The clock in the top strip runs from 22:00 and '
          'moves faster than real time.'),
      ManualEntry('INSTRUMENTS',
          'The meters are damped, like real gauges — they lag the process by a '
          'second or so and settle rather than snapping. Readouts are in proper '
          'units: decades of flux, °C, bar, MWe.'),
    ],
  ),
  ManualSection(
    'SANITY, THE CANTEEN AND THE SCREAMING',
    'You are a person, not a subsystem. Sanity starts at 100 and falls all '
        'shift — slowly when the plant is calm, much faster with alarms '
        'flashing, damage on the core or the horn going. And somewhere in the '
        'building something screams. Nobody has ever found it. Every time you '
        'hear it you lose 5 sanity. At zero you put the clipboard down and walk '
        'out, which ends the shift and costs most of your research.',
    [
      ManualEntry('SANITY BAR',
          'Always on screen under the uranium readout. Violet is fine, amber is '
          'tired, red is close to done.'),
      ManualEntry('SIP',
          'The button beside the bar. One tap takes a sip of whatever suits the '
          'gap best. The number next to it is total servings in hand.'),
      ManualEntry('CANTEEN',
          'Buy food and drink with uranium in UPGRADES → CANTEEN. Stock keeps '
          'between shifts. BLACK START coffee is three sips at fifteen each; '
          'NIGHT SHIFT noodles are one big bowl; HEAVY WATER is five small ones.'),
      ManualEntry('CREW PANEL',
          'The sixth console panel. Shows sanity, stress load, everything you '
          'are carrying, and which AI crew are on duty.'),
      ManualEntry('EAR DEFENDERS',
          'Do not stop the screaming. Reduce what it costs you by 40% a level.'),
    ],
  ),
  ManualSection(
    'THE AI CREW',
    'Bots are bought once with uranium and work every shift after. They do one '
        'job each, exactly as a competent operator would, and they never get '
        'tired.',
    [
      ManualEntry('ROD-9', 'Drives the rod banks to your power setpoint.'),
      ManualEntry('PZR-4', 'Holds pressure at 155 bar on heaters and spray.'),
      ManualEntry('SYNC-3', 'Closes the generator breaker in phase for you.'),
      ManualEntry('FEED-2', 'Trims the feed pump to hold steam generator level.'),
      ManualEntry('WATCH-1', 'Acknowledges the annunciator and kills the horn.'),
      ManualEntry('BARISTA-B',
          'Watches you, not the plant. Serves a sip whenever sanity drops '
          'below 35 — as long as there is something in the canteen.'),
    ],
  ),
  ManualSection(
    'THE ANNUNCIATOR',
    'The tile wall above the console. Flashing means a new condition, steady '
        'means you have seen it, dark means the plant is content.',
    [
      ManualEntry('ACK', 'Acknowledges every flashing tile and silences the horn.'),
      ManualEntry('SIL', 'Cuts the horn but leaves the tiles flashing.'),
      ManualEntry('RST', 'Clears acknowledged tiles whose condition has gone away.'),
      ManualEntry('TEST', 'Lights every tile so you can spot a dead one.'),
    ],
  ),
  ManualSection(
    'HOW YOU ACTUALLY LOSE',
    'Not to a timer. Core damage accumulates only when fuel exceeds 1200 °C, '
        'pressure exceeds 185 bar, or the steam generators run dry with heat '
        'still in the core. The protection system trips you long before any of '
        'that, and a trip is survivable — you restart and carry on. Reaching '
        '100% damage destroys the core and ends the shift.',
    [
      ManualEntry('WHEN IN DOUBT',
          'SCRAM. A trip costs you minutes. A meltdown costs you everything.'),
    ],
  ),
];
...
class Game {
  ...
  bool canAfford(ShopItem it) {
    ...
    return it.research ? research >= costOf(it) : uranium >= costOf(it);
  }
  ...
  /// Next scream is 25–75 seconds out, so it never becomes a metronome.
  void scheduleScream() => screamIn = 25 + rng.nextDouble() * 50;
  ...
  void load() {
    ...
    try {
      ...
      uranium = (m['ur'] as num?)?.toDouble() ?? (m['cr'] as num?)?.toDouble() ?? 0;
      research = (m['rs'] as num?)?.toInt() ?? 0;
      plant.burnup = (m['bu'] as num?)?.toDouble() ?? 0;
      refuels = (m['rf'] as num?)?.toInt() ?? 0;
      final pan = m['pan'];
      if (pan is Map) {
        pan.forEach((k, v) {
          if (v is num) pantry['$k'] = v.toInt();
        });
      }
      final up = m['up'];
      if (up is Map) {
      ...
      lifetimeMwh = (m['mwh'] as num?)?.toDouble() ?? 0;
      bestShiftMwh = (m['best'] as num?)?.toDouble() ?? 0;
      shifts = (m['sh'] as num?)?.toInt() ?? 0;
      meltdowns = (m['md'] as num?)?.toInt() ?? 0;
      trips = (m['tr'] as num?)?.toInt() ?? 0;
      sfx.muted = (m['mu'] as bool?) ?? false;
      lastMwe = (m['lm'] as num?)?.toDouble() ?? 0;
...
class HomeScreen extends StatelessWidget {
          ...
          child: ListView(
            ...
            children: [
                ...
                children: [
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text('MELTDOWN',
                            style: ts(34, cGreen, w: FontWeight.w900, ls: 5)
                                .copyWith(shadows: [
                              Shadow(
                                  color: cGreen.withValues(alpha: 0.55),
                                  blurRadius: 22)
                            ])),
                        Text('PRESSURIZED WATER REACTOR — CONTROL ROOM',
                            style: ts(9, cInkDim, ls: 2)),
                      ],
                    ),
                  ),
                  _iconBtn(g.sfx.muted ? '∅' : '♪', () {
              ...
              PanelBox(
                child: Row(
                  children: [
                    _stat('URANIUM', '⬢ ${fmt(g.uranium)}', cGold),
                    _stat('RESEARCH', '◆ ${g.research}', cViolet),
                    _stat('LIFETIME MWh', fmt(g.lifetimeMwh), cCyan),
                  ],
                ),
              ),
              ...
              PanelBox(
                child: Row(
                  children: [
                    _stat('SHIFTS', '${g.shifts}', cInk),
                    _stat('BEST MWh', fmt(g.bestShiftMwh), cInk),
                    _stat('TRIPS', '${g.trips}', cAmber),
                    _stat('MELTDOWNS', '${g.meltdowns}', cRed),
                  ],
                ),
              ),
              if (g.offlineGain > 0) ...[
                ...
                PanelBox(
                  ...
                  child: Row(
                    children: [
                      const Text('⧗', style: TextStyle(fontSize: 18, color: cViolet)),
                      const SizedBox(width: 10),
                      Expanded(
                        child: Text(
                          'Remote dispatch earned ⬢${fmt(g.offlineGain)} while you '
                          'were away for ${formatDur(g.offlineAway)}.',
                          style: ts(11, cInk, w: FontWeight.w600),
                        ),
                      ),
                    ],
                  ),
              ...
              const SizedBox(height: 16),
              BigButton(
                label: g.shiftActive ? 'RESUME SHIFT' : 'START SHIFT',
                sub: g.lvl('hotStart') > 0
                    ? 'Hot standby handover — plant is already critical'
              ...
              BigButton(
                label: 'UPGRADES',
                sub: '${kShop.length} systems · ${g.affordableCount} affordable now',
                glyph: '⚙',
                color: cGold,
              ...
              BigButton(
                ...
                sub: 'What every switch, meter and alarm actually does',
                glyph: '▥',
                color: cBlue,
                onTap: () {
                  g.sfx.softClick();
              ...
              Text(
                'The plant is stable when balanced and the protection system '
                'trips before it breaks — losing the core takes real neglect. '
                'Losing yourself is easier. Sanity falls all shift and drops 5 '
                'every time the building screams, so keep the canteen stocked.',
                style: ts(10.5, cInkFaint, w: FontWeight.w500, ls: 0.2),
              ),
  ...
  Widget _iconBtn(String glyph, VoidCallback onTap) => GestureDetector(
        onTap: onTap,
        child: Container(
          width: 34,
          height: 34,
          ...
          child: Text(glyph, style: ts(14, cInkDim)),
        ),
      );
        ...
        child: Column(
          ...
          children: [
            Text(label,
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
                style: ts(8, cInkFaint, ls: 1.1)),
            const SizedBox(height: 2),
            FittedBox(
              fit: BoxFit.scaleDown,
              alignment: Alignment.centerLeft,
              child: Text(value, style: ts(14, color, w: FontWeight.w900)),
            ),
          ],
        ),
      );
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: Row(
                  children: [
                    Text('▥', style: ts(19, cBlue, w: FontWeight.w900)),
                    const SizedBox(width: 10),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          ...
                          Text('every control, what it does, and why',
                    ...
                    return Padding(
                      ...
                      child: PanelBox(
                        child: Column(
                          ...
                          children: [
                            Text(s.title,
                                style: ts(12, cGreen, w: FontWeight.w900, ls: 1.6)),
                            const SizedBox(height: 6),
                            Text(s.blurb,
                                style: ts(11, cInk,
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: LayoutBuilder(builder: (context, box) {
        ...
        final avail =
            math.max(90.0, box.maxHeight - (topH + sanH + annH + objH + tabsH + bottomH));
