...
void main() {
  ...
  group('game shell', () {
    testWidgets('boots to the home screen and starts a shift', (tester) async {
      ...
      expect(find.text('MELTDOWN'), findsOneWidget);
  ...
  group('sanity, the canteen and the screaming', () {
    ...
    testWidgets('screams are scheduled, not constant', (tester) async {
      await tester.pumpWidget(const ReactorApp());
      final g = tester.state<GameRootState>(find.byType(GameRoot)).game;
      g.startShift();
      expect(g.screamIn, greaterThanOrEqualTo(25));
      expect(g.screamIn, lessThanOrEqualTo(75));
    });
